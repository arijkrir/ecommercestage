...
export class ProfileComponent implements OnInit {
  ...
  showPassword = false;
  displaySuccessDialog: boolean = false; // Assurez-vous que le type est boolean
    ...
    if (userInfo && userInfo.id) {
      this.userService.updateUser(userInfo.id, this.user).subscribe(
        (response) => {
          ...
          this.displaySuccessDialog = true; // Assignez une valeur boolean
        },
        (error) => {
  ...
  onCloseSuccessDialog(): void {
    this.displaySuccessDialog = false; // Assignez une valeur boolean
  }
}

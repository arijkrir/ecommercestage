...
export class LivraisonComponent implements OnInit {
  ...
  constructor(private authService: AuthService, private orderService: OrderService, private cartService: CartService, private router: Router) {
    this.steps = [
  ...
  confirmOrder(): void {
    if (this.validatePersonalInfo() && this.validateDeliveryInfo()) {
      const orderDetails = {
        articles: this.cartItems.map(item => ({
          produitId: item.id,
          quantiteeCommandee: item.quantity
        })),
        userId: this.authService.getUserInfo().id,
        prixTotal: this.totalAfterPromo // Assurez-vous que c'est bien totalAfterPromo
      };
  
      this.orderService.confirmOrder(orderDetails).subscribe(response => {
        console.log('Commande confirmée!', response);
        this.router.navigate(['/confirmation']); // Redirigez vers une page de confirmation
      }, error => {

...
export class LivraisonComponent implements OnInit {
  ...
  confirmOrder(): void {
    if (this.validatePersonalInfo() && this.validateDeliveryInfo()) {
      ...
      // Mettre à jour les quantités sur le FTP avant de confirmer la commande
      this.orderService.updateFtpQuantities(this.cartItems).subscribe(
        () => {
          // Si la mise à jour FTP est réussie, procéder à la confirmation de la commande
        ...
        error => {
          console.error('Erreur lors de la mise à jour des quantités sur le FTP', error);
          this.confirmationMessage = 'Erreur lors de la mise à jour des quantités sur le FTP';
          this.displayModal = true;  // Afficher le dialogue
        }
      );
    }  

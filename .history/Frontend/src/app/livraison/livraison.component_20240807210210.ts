...
@Component({
  ...
  providers: [MessageService]  // Ajouter MessageService comme provider
})
  ...
  confirmOrder(): void {
    if (this.validatePersonalInfo() && this.validateDeliveryInfo()) {
      ...
      this.orderService.confirmOrder(orderDetails).subscribe(response => {
        ...
        this.messageService.add({severity: 'success', summary: 'Succès', detail: 'Commande confirmée avec succès!'});
      }, error => {
        console.error('Erreur lors de la confirmation de la commande', error);
        this.messageService.add({severity: 'error', summary: 'Erreur', detail: 'Erreur lors de la confirmation de la commande'});
      });
    }
  }
}

...
export class ProductlistScrollableComponent implements OnInit {
  ...
  loadFilteredProducts(term: string, category: string): void {
    this.productService.getCategoryProductIds(category, this.offset, this.limit).subscribe(
      (ids: number[]) => {
        ...
        this.loadCategoryProductDetails(ids, category);
      },
      (error) => {
  ...
  loadCategoryProductDetails(ids: number[], category: string): void {
    this.productService.getCategoryProductDetails(ids, category, 'details').subscribe(
      (produits: Produit[]) => {
        ...
        this.productService.getCategoryProductDetails(ids, category, 'csv').subscribe(
          (produits: Produit[]) => {

...
export class ProductlistScrollableComponent implements OnInit {
  ...
  loadCategoryProducts(): void {
    ...
    this.productService.getCategoryProductIds(this.category, this.ofset, this.limit).subscribe(
      (ids: number[]) => {
        if (ids.length < 1000) {
...
if (source === 'csv') {
  ...
  onScroll(): void {
    ...
    if (!this.loading && this.hasMoreProduits) {
      if (this.category) {
        // Si une catégorie est sélectionnée, charger les produits de la catégorie
        this.loadCategoryProducts();
      } else {
          this.loadProductIds();
      }

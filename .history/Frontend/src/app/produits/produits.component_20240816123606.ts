...
export class ProductlistScrollableComponent implements OnInit {
    ...
    this.productService.getProductDetailsFromCsvByIds(ids).subscribe(
      (produits: Produit[]) => {
        produits.forEach(csvProduct => {
          const existingProduct = this.produits.find(product => product.id === csvProduct.id);
          if (existingProduct) {
  ...
  updateQuantity(produit: Produit, quantity: number): void {
    produit.quantity = quantity;
    this.cartService.updateQuantity(produit);
    this.saveCartItemsToLocalStorage();
  }

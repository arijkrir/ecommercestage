...
export class ProductlistScrollableComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.route.queryParams.subscribe(params => {
      ...
      if (searchTerm) {
        // Appeler les trois méthodes de recherche en parallèle
        forkJoin({
          details: this.productService.searchProducts(searchTerm),
          csv: this.productService.searchProductscsv(searchTerm),
          ftp: this.productService.searchProductsftp(searchTerm)
        }).subscribe(
          ({ details, csv, ftp }) => {
            // Fusionner les résultats
            const allResults = [...details, ...csv, ...ftp];
            this.produits = this.removeDuplicates(allResults); // Assurer l'unicité des produits si nécessaire
          },
          (error) => {
        ...
        this.loadProductIds(); // Charger tous les produits si aucun terme de recherche
      }
    });
  }
  
  removeDuplicates(products: Produit[]): Produit[] {

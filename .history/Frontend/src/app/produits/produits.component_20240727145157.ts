        ...
        switch (source) {
          case 'csv':
            this.productService.searchProductscsv(searchTerm).subscribe(
              (results: Produit[]) => {
                // Met à jour les produits avec les résultats CSV
          ...
          case 'ftp':
            this.productService.searchProductsftp(searchTerm).subscribe(
              (results: Produit[]) => {
                // Met à jour les produits avec les résultats FTP
                this.produits = results;
          ...
          default:
            this.productService.searchProducts(searchTerm).subscribe(
              (results: Produit[]) => {
                // Met à jour les produits avec les résultats de détails
                this.produits = results;
                console.log('Search Results:', this.produits);
                
                // Charger les détails de produits CSV et FTP
    ...
    this.productService.getProductDetails(ids).subscribe(
      (produits: Produit[]) => {
        ...
        produits.forEach(newProduct => {
          ...
          if (existingProduct) {
            existingProduct.quantity = 1; // Initialiser la quantité si nécessaire
          } else {

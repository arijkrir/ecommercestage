...
export class ProductService {
 ...
 searchProducts(term: string): Observable<Produit[]> {
  return this.http.get<Produit[]>(`${this.apiUrl}/search-with-details?query=${term}`);
}

searchProductscsv(term: string): Observable<Produit[]> {
  return this.http.get<Produit[]>(`${this.apiUrl}/search-csv-details?query=${term}`);
}

searchProductsftp(term: string): Observable<Produit[]> {
  return this.http.get<Produit[]>(`${this.apiUrl}/search-ftp-details?query=${term}`);
}
}

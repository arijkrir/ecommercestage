...
export class CartComponent implements OnInit {
  ...
  increaseQuantity(produit: Produit): void {
    if (produit.stock !== undefined && produit.quantity < produit.stock) { // Vérifiez que la quantité ne dépasse pas le stock
      produit.quantity++;
  ...
  updateAvailability(): void {
    ...
    this.cartItems.forEach(produit => {
      produit.available = produit.stock !== undefined && produit.stock > 0;
    });
  }
}

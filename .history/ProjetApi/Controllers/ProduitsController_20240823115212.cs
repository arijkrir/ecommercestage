...
public async Task<ActionResult<IEnumerable<object>>> GetUpsellingProducts([FromQuery] int productId)
{
    ...
    // Filtrer les produits ayant la même image et un prix supérieur
    var similarProducts = (from p in _context.Produits
                           join csv in csvProducts on p.Id equals csv.Id
                           where p.Image == selectedProduct.Image && csv.PromoPrice > csvProducts.First(x => x.Id == productId).PromoPrice
                           orderby csv.PromoPrice descending
                           select new
                           {
                               p.Id,
                               csv.PromoPrice

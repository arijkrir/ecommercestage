...
public async Task<ActionResult<IEnumerable<object>>> GetCrossSellingProducts([FromQuery] int productId)
{
    ...
    // Trouver le produit sélectionné dans la base de données
    var selectedProduct = await _context.Produits.FindAsync(productId);
    if (selectedProduct == null)
    {
        return NotFound("Produit non trouvé");
    ...
    // Déterminer à quel groupe appartient l'image du produit sélectionné
    List<string> selectedGroup = null;
    if (group1.Contains(selectedProduct.Image)) selectedGroup = group1;
    else if (group2.Contains(selectedProduct.Image)) selectedGroup = group2;
    else if (group3.Contains(selectedProduct.Image)) selectedGroup = group3;
    else if (group4.Contains(selectedProduct.Image)) selectedGroup = group4;

    if (selectedGroup == null)
    {
        return NotFound("Le produit ne fait pas partie d'un groupe d'images connu");
    }

    // Récupérer les autres produits appartenant au même groupe d'images
    ...
    // Limiter à 5 produits et retourner les informations nécessaires
    var crossSellingProductsResult = crossSellingProducts
        .Take(5)
        .Select(p => new 
        {
            p.Id,
    ...
    return Ok(crossSellingProductsResult);
}

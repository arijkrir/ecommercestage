...
public async Task<IActionResult> ConfirmOrder([FromBody] OrderRequest orderRequest)
{
    ...
    await _context.SaveChangesAsync(); // Assurez-vous que la commande est ajoutée
    ...
    foreach (var article in orderRequest.Articles)
    {
        var articleEntity = new Article
        {
            ...
            CommandeId = commande.CommandeId // Associer l'article à la commande
        };

        _context.Articles.Add(articleEntity);
    }

    // Enregistrer les changements pour les articles
    await _context.SaveChangesAsync();
...
{
    // Compose the email
    var fromAddress = new MailAddress("your-email@example.com", "Your E-Commerce Site");
    ...
    foreach (var article in orderRequest.Articles)
    {
        ...
        if (product != null)
        {
            body += $"- {product.Designation}: {article.QuantiteeCommandee} x {product.Prix:C}\n";
        }
    }

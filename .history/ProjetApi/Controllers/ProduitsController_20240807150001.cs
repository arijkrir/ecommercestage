...
[HttpPost("confirm")]
public async Task<IActionResult> ConfirmOrder([FromBody] OrderDetailsDto orderDetails)
{
    ...
    var command = new Commande
    {
        ...
        PrixTotal = orderDetails.TotalAmount // Assurez-vous que c'est bien PrixTotal
    };

    _context.Commandes.Add(command);
    await _context.SaveChangesAsync();

    // Ajoutez les articles à la commande
    foreach (var item in orderDetails.CartItems) // Assurez-vous que le nom correspond
    {
        var CartItems = new CartItems
        {
            ProduitId = item.ProduitId, // Vérifiez que ce nom est correct
            Quantite = item.QuantiteeCommandee, // Vérifiez que ce nom est correct
            CommandeId = command.CommandeId
        };
        _context.CartItemss.Add(CartItems);
    }
    await _context.SaveChangesAsync();

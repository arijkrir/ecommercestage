...
public async Task SendConfirmationEmail(User user, OrderRequest orderRequest)
{
    var smtpSettings = _configuration.GetSection("SmtpSettings").Get<SmtpSettings>();

    var message = new MimeMessage();
    message.From.Add(new MailboxAddress("Your E-Commerce Site", smtpSettings.Username));
    message.To.Add(new MailboxAddress($"{user.Nom} {user.Prenom}", user.Email));
    message.Subject = "Order Confirmation";

    var bodyBuilder = new BodyBuilder
    {
        TextBody = $"Dear {user.Prenom},\n\n" +
    ...
    foreach (var article in orderRequest.Articles)
    {
        ...
        if (product != null)
        {
            bodyBuilder.TextBody += $"- {product.Designation}: {article.QuantiteeCommandee} \n";
        }
    }

...
{
    if (orderRequest == null || !orderRequest.Articles.Any())
    {
    ...
    try
    {
        ...
        var commande = new Commande
        {
            ...
            Articles = orderRequest.Articles.Select(a => new Article
            {
                ProduitId = a.ProduitId,
                QuantiteeCommandee = a.QuantiteeCommandee
                // Remarquez que CommandeId n'est pas encore défini ici
            }).ToList()
        };

        // Assigner la commande aux articles après que la commande a été ajoutée
        foreach (var article in commande.Articles)
        {
            article.CommandeId = commande.CommandeId; // Cela peut générer une erreur car CommandeId n'est pas encore défini
        }

        // Ajouter la commande et les articles au contexte
        _context.Commandes.Add(commande);
        await _context.SaveChangesAsync();

        // Retourner l'ID de la commande pour vérification
        return Ok(new { Message = "Order confirmed successfully!", CommandeId = commande.CommandeId });
    }
    catch (Exception ex)
    {
        // Log l'exception
        // Pour la production, utilisez un système de journalisation comme Serilog, NLog, etc.

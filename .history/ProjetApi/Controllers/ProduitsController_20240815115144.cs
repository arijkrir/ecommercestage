...
public async Task<ActionResult<IEnumerable<int>>> GetCategoryProductsByIds(string category, [FromQuery] int offset, [FromQuery] int limit)
{
    if (offset < 0 || limit <= 0)
    {
    ...
    Console.WriteLine($"Category: {category}, Offset: {offset}, Limit: {limit}");
...
[HttpPost("GetCategoryProductDetailsByIds")]

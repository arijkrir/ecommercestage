...
public async Task<IActionResult> ConfirmOrder([FromBody] OrderDetailsDto orderDetails)
{
    ...
    var command = new Commande
    {
        ...
        PrixTotal = orderDetails.TotalAmount // Utiliser la propriété TotalAmount du DTO
    };
    ...
    foreach (var item in orderDetails.CartItems)
    {
        ...
        {
            ProduitId = item.ProduitId, // Assurez-vous que ce nom est correct
            Quantite = item.Quantity, // Utiliser la propriété Quantity du DTO
            CommandeId = command.CommandeId
        };
        _context.Articles.Add(article); // Assurez-vous que 'Articles' est le nom correct du DbSet
    }
    await _context.SaveChangesAsync();

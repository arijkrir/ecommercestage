...
public async Task<ActionResult<IEnumerable<ProductCsv>>> GetProductDetailsFromCsvByIds([FromBody] List<int> ids)
{
    try
    {
        ...
        {
            // Read data from the CSV file if not cached
            var path = @"C:\Users\arijk\Desktop\ecommercestage\ProjetApi\products.csv";
            using (var reader = new StreamReader(path))
            using (var csv = new CsvReader(reader, new CsvConfiguration(CultureInfo.InvariantCulture) { Delimiter = "," }))
            {
                products = csv.GetRecords<ProductCsv>().ToList();
            }

            // Cache the data for future requests
            var cacheEntryOptions = new DistributedCacheEntryOptions()
                .SetSlidingExpiration(TimeSpan.FromHours(1)); // Adjust cache expiration as needed
        ...
        else
        {
            ...
            products = JsonConvert.DeserializeObject<List<ProductCsv>>(cachedData);
        }

        var filteredProducts = products.Where(r => ids.Contains(r.Id)).ToList();

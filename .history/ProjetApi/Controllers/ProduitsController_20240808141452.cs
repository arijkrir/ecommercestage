   ...
   public class ProduitsController : ControllerBase
    {
        ...
        public ProduitsController(IEmailService emailService,ApplicationDbContext context, IDistributedCache cache)
        {
            _context = context;
            _cache = cache;
...
public async Task<ActionResult<IEnumerable<ProduitFTP>>> SearchFtpDetails([FromBody] SearchRequest request)
{
 ...
 [HttpPost("confirm")]
    public async Task<IActionResult> ConfirmOrder([FromBody] OrderRequest orderRequest)
    {
        ...
        var body = $"<h1>Order Confirmation</h1><p>Thank you for your order!</p>" +
                   $"<p>Order ID: {commande.CommandeId}</p>" +
                   $"<p>Total Price: {orderRequest.PrixTotal:C}</p>" +
                   "<h2>Order Details:</h2>";

        foreach (var article in orderRequest.Articles)
        {
            body += $"<p>Product ID: {article.ProduitId}, Quantity: {article.QuantiteeCommandee}</p>";
        }

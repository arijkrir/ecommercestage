...
[HttpPost("GetCategoryProductsByIds/{category}")]
public async Task<ActionResult<IEnumerable<Produits>>> GetCategoryProductsByIds(string category, [FromBody] List<int> ids)
{
    var products = await _context.Produits
        .Where(p => p.Category == category && ids.Contains(p.Id))
        .ToListAsync();
...
public ActionResult<IEnumerable<ProductCsv>> GetCategoryProductDetailsFromCsvByIds(string category, [FromBody] List<int> ids)
{
    try
    {
        var path = @"C:\path\to\your\products.csv";
        using (var reader = new StreamReader(path))
        using (var csv = new CsvReader(reader, new CsvConfiguration(CultureInfo.InvariantCulture) { Delimiter = "," }))
        {
            var records = csv.GetRecords<ProductCsv>().Where(p => p.Category == category).ToList();
            var products = records.Where(r => ids.Contains(r.Id)).ToList();

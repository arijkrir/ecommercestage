...
public async Task<IActionResult> ConfirmOrder([FromBody] OrderRequest orderRequest)
{
    ...
    var commande = new Commande
    {
        ...
        DateLivraison = DateTime.Now.AddDays(3) // Par exemple, livraison après 3 jours
    };
    ...
    await SendConfirmationEmail(user, orderRequest, commande.DateLivraison);

    return Ok(new { message = "Order confirmed successfully!" });
}

// Method to send confirmation email
private async Task SendConfirmationEmail(User user, OrderRequest orderRequest, DateTime dateLivraison)
{
    ...
    string body = $"Dear {user.Prenom},\n\n" +
                  ...
                  $"Delivery Date: {dateLivraison.ToString("dd/MM/yyyy")}\n\n" +
                  "Items:\n";

...
private async Task SendConfirmationEmail(User user, OrderRequest orderRequest)
{
 ...
 [HttpPost("update-stock-from-ftp")]
        public async Task<IActionResult> UpdateStockFromFtp()
        {
            try
            {
                var ftpHost = "127.0.0.1"; // Replace with your FTP server host
                var ftpUsername = "arij"; // Replace with your FTP server username
                var ftpPassword = "arij"; // Replace with your FTP server password
                var remoteFilePath = "/csv/quantity.csv"; // Path to the CSV file on the FTP server
                ...
                {
                    return NotFound("No product quantities found in the CSV file.");
                }

                // Update product quantities in the database
                foreach (var productQuantity in productQuantities)
                {
                    var product = await _context.ProduitFTP.FindAsync(productQuantity.ID);
                    if (product != null)
                    {
                        ...
                        {
                            product.Quantity = 0; // Ensure the quantity does not go below 0
                        }
                ...
                return Ok("Stock quantities updated successfully.");
            }
            catch (Exception ex)
            {
                return StatusCode(500, $"Internal server error: {ex.Message}");

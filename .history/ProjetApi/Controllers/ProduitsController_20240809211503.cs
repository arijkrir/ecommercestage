...
public async Task<IActionResult> UpdateFtpQuantities([FromBody] List<ProductUpdateRequest> productUpdates)
{
    try
    {
        ...
        // Using FTP connection only once for performance
        List<ProduitFTP> productQuantities;
        ...
        using (var ftpClient = new FtpClient(ftpHost, ftpUsername, ftpPassword))
        {
            ...
            // Directly read the file into memory
            using (var memoryStream = new MemoryStream())
            {
                ftpClient.Download(memoryStream, remoteFilePath);
            ...
            // Directly write the updated data to memory
            using (var memoryStream = new MemoryStream())
            ...
            using (var csv = new CsvWriter(writer, new CsvConfiguration(CultureInfo.InvariantCulture) { Delimiter = "," }))
            {
                ...
                // Upload the updated file from memory to the FTP server
                ftpClient.Upload(memoryStream, remoteFilePath);
            }

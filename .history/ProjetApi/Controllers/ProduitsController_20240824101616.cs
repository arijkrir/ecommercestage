...
public async Task<ActionResult<IEnumerable<object>>> GetCrossSellingProducts([FromQuery] int productId)
{
    ...
    var selectedProduct = await _context.Produits.AsNoTracking().FirstOrDefaultAsync(p => p.Id == productId);
    if (selectedProduct == null)
    {
        return NotFound("Produit non trouvé");
    }

    Console.WriteLine($"Image du produit sélectionné : {selectedProduct.Image}");
    ...
    if (imageGroupId == 0)
    {
        Console.WriteLine($"Aucun groupe d'images trouvé pour l'image : {selectedImage}");
    ...
    var crossSellingProductsFromDb = await _context.Produits
        .AsNoTracking()
        .Where(p => groupImageList.Contains(p.Image) && p.Image != selectedImage && p.Id != productId)
        .Skip((pageNumber - 1) * pageSize)
         .Take(pageSize)
        .ToListAsync();
    ...
    try
    {
        ...
        using (var csv = new CsvReader(reader, new CsvConfiguration(CultureInfo.InvariantCulture) { Delimiter = "," }))
        {
            await foreach (var record in csv.GetRecordsAsync<ProductCsv>())
            {
                csvProducts.Add(record);
            }
        }
    }
    catch (Exception ex)

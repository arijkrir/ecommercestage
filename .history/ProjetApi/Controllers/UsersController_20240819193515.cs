...
namespace ProjetApi.Controllers
{
    ...
    {
        var topProducts = _context.Articles
            .GroupBy(c => c.ProduitId)
            .Select(g => new
            {
                ProduitId = g.Key,
                TotalQuantity = g.Sum(c => c.QuantiteeCommandee)
            })
            .OrderByDescending(x => x.TotalQuantity)
            .Take(5)
            .Join(_context.Produits,
                  topProduct => topProduct.ProduitId,
                  produit => produit.Id,
                  (topProduct, produit) => new
                  {
                      produit.Designation,
                      produit.Reference,
                      produit.Image,
...
{
    var topCategories = _context.Articles
        .GroupBy(c => c.ProduitId)
        .Select(g => new
        {
            ProduitId = g.Key,
        ...
        {
            Categorie = g.Key,
            TotalVentes = g.Sum(x => x.TotalQuantity)
        })

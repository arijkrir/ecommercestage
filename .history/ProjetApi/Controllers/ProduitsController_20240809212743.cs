...
public async Task<IActionResult> UpdateFtpQuantities([FromBody] List<ProductUpdateRequest> productUpdates)
{
    try
    {
        ...
        using (var ftpClient = new FtpClient(ftpHost, ftpUsername, ftpPassword))
        {
            ...
            var localFilePath = Path.GetTempFileName();
            await ftpClient.DownloadFileAsync(localFilePath, remoteFilePath);

            using (var fileStream = new FileStream(localFilePath, FileMode.Open, FileAccess.Read))
            using (var reader = new StreamReader(fileStream))
            using (var csv = new CsvReader(reader, new CsvConfiguration(CultureInfo.InvariantCulture) { Delimiter = "," }))
            {
                productQuantities = await csv.GetRecordsAsync<ProduitFTP>().ToListAsync();
            }
            ...
            // Write the updated data back to the CSV asynchronously
            using (var writer = new StreamWriter(localFilePath))
            using (var csv = new CsvWriter(writer, new CsvConfiguration(CultureInfo.InvariantCulture) { Delimiter = "," }))
            {
                await csv.WriteRecordsAsync(productQuantities);
            }

            // Upload the updated file back to the FTP server asynchronously

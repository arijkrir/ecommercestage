...
public async Task<IActionResult> ConfirmOrder([FromBody] OrderRequest orderRequest)
{
    ...
    var orderConfirmation = new OrderConfirmationDto
    {
        ...
        Items = orderRequest.Articles.Select(a => new OrderItemDto
        {
            ProductName = (await _context.Produits.FindAsync(a.ProduitId)).Designation,
            Quantity = a.QuantiteeCommandee
        }).ToList(),
        TotalPrice = orderRequest.PrixTotal,
        // DeliveryDate = DateTime.Now.AddDays(3) // Example delivery date
    };

    // Send a confirmation email
    var gmailService = new GmailService("YOUR_ACCESS_TOKEN"); // Use your access token
    await gmailService.SendEmailAsync(orderConfirmation);
